...
## Порядок тестирования через Swagger (для скриншотов)

1. Откройте `/swagger` — сделайте скриншот со списком методов.
2. **POST** `/api/tasks` — создайте минимум 3 задачи по очереди.
3. **GET** `/api/tasks` — получите список всех задач, сделайте скриншот.
4. **GET** `/api/tasks/{id}` — получите одну задачу по её Id.
5. **PUT** `/api/tasks/{id}` — измените название или статус задачи, сделайте скриншот.
6. **DELETE** `/api/tasks/{id}` — удалите одну задачу.
7. Повторите GET `/api/tasks` — убедитесь, что изменения применились.
